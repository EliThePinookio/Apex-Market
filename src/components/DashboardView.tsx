...
import { getDashboardAutonomousBriefing, getStoredBrainModel } from '../services/centralBrainService';
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  // Central Brain Proactive Telemetry State
  const [brainBriefing, setBrainBriefing] = useState<string | null>(null);
  const [isBrainLoading, setIsBrainLoading] = useState(false);
  const [brainModelUsed, setBrainModelUsed] = useState<string>('GLM 5.2');

  const fetchBrainTelemetry = async () => {
    setIsBrainLoading(true);
    try {
      const res = await getDashboardAutonomousBriefing(summary, products, transactions, profile);
      setBrainBriefing(res.result);
      setBrainModelUsed(res.model.split('/')[1] || res.model);
    } catch (e) {
      console.warn('Central brain proactive briefing fetch error', e);
    } finally {
  ...
  useEffect(() => {
    fetchBrainTelemetry();
  }, [summary.totalRevenue, products.length, transactions.length]);
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      {/* 0. CENTRAL BRAIN AUTONOMOUS INTELLIGENCE BANNER */}
      <div className="p-4 sm:p-5 rounded-3xl bg-gradient-to-r from-blue-900/20 via-indigo-900/20 to-slate-900/30 dark:bg-gradient-to-r dark:from-blue-950/40 dark:via-indigo-950/40 dark:to-slate-950/60 border border-blue-500/25 backdrop-blur-xl text-slate-900 dark:text-slate-100 shadow-sm relative overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-blue-500/15 pb-3">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-cyan-500 text-white flex items-center justify-center shrink-0 shadow-md shadow-blue-500/25">
              <Brain className="w-5 h-5 animate-pulse" />
            </div>
            <div>
              <div className="flex items-center space-x-2">
                <h2 className="text-sm sm:text-base font-extrabold text-slate-900 dark:text-white tracking-tight">
                  Central Brain Telemetry
                </h2>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-blue-500/15 text-blue-700 dark:text-blue-300 border border-blue-500/25">
                  OpenRouter • {brainModelUsed}
                </span>
              </div>
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5 font-semibold flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping inline-block"></span>
                Autonomous analysis running on active store telemetry
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 self-start sm:self-auto">
            <button
              onClick={fetchBrainTelemetry}
              disabled={isBrainLoading}
              className="p-2 rounded-2xl bg-white/70 dark:bg-white/[0.06] hover:bg-white dark:hover:bg-white/[0.1] text-slate-700 dark:text-slate-200 border border-slate-200/80 dark:border-white/[0.08] text-xs font-bold transition active:scale-95 cursor-pointer disabled:opacity-50"
              title="Refresh Central Brain Analysis"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${isBrainLoading ? 'animate-spin' : ''}`} />
            </button>

            {onOpenCentralBrain && (
              <button
                onClick={onOpenCentralBrain}
                className="px-3.5 py-2 rounded-2xl bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 hover:from-blue-500 hover:to-violet-500 active:scale-[0.97] text-white font-bold text-xs flex items-center space-x-1.5 shrink-0 cursor-pointer transition shadow-sm"
              >
                <Sparkles className="w-3.5 h-3.5" />
                <span>Open Brain Assistant</span>
              </button>
            )}
          </div>
        </div>

        {/* Live Autonomous Intelligence Feed */}
        <div className="mt-3.5">
          {isBrainLoading ? (
            <div className="py-3 flex items-center space-x-2 text-xs text-slate-500 dark:text-slate-400">
              <span className="w-2 h-2 rounded-full bg-blue-500 animate-bounce"></span>
              <span>OpenRouter Central Brain calculating today's store briefing...</span>
            </div>
          ) : brainBriefing ? (
            <div className="text-xs sm:text-sm text-slate-700 dark:text-slate-200 leading-relaxed font-medium space-y-1.5 whitespace-pre-line bg-white/40 dark:bg-white/[0.02] p-3 rounded-2xl border border-blue-500/10">
              {brainBriefing}
            </div>
          ) : (
            <div className="text-xs text-slate-500 dark:text-slate-400">
              Telemetry active. Keep replenishing low inventory and maintaining steady customer checkout velocity.
            </div>
          )}
        </div>
      </div>
